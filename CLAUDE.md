...
Quantumania's `orderNote` is deliberately provisional: the user wants to re-order some of Phase Four once the rest of it is written, so it states the Loki-either-side reasoning and nothing more. **What If...? is threaded through `optionalViewing` one season at a time**, so a reader never gets pointed at a season that spoils films they haven't reached: S1 on Quantumania, S2 on Multiverse of Madness, S3 on Ironheart. **Phases Five and Six are not** — they're still original AI prose and hold the last 28 em dashes (`npm run check` reports the count per phase). Phase Six's three released films and Phase Five's eleven entries are the remaining work.
