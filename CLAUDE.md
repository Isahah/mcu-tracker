...
**Secret Invasion sits before The Marvels, and it is a judgement call, not a fact.** Sources genuinely disagree and the two productions do not interlock. The argument used: Secret Invasion ends with Fury going back to space and The Marvels has him already stationed there. That uncertainty is written into Secret Invasion's own `orderNote` on purpose. The one thing that would settle it is whether Talos appears alive in The Marvels, since he dies in Secret Invasion.
...
- **Places and organisations in the database.** Discussed August 2026, agreed approach: same `characters.json` rather than a separate file, since the schema already fits and Alioth, Goose and Khonshu are already not people. Needs a `type` field (absent = person, so the existing 144 stay untouched), a filter on the database screen so browsing does not degrade, and a copy pass on "Character Database" and the **PERSONNEL FILE** header, neither of which suits a place. Expect only 10–15 `watchFor` chips to newly resolve; of the 113 chips that currently link to nothing, most are concepts that are meant to stay plain.
